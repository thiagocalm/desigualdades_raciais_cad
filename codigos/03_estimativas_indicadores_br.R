...
#' @description Estimativas dos indicadores a nível do Brasil
#' -----------------------------------------------------
...
indicadores_br <- indicadores_municipais |>
  group_by(ano) |>
  reframe(
    across(c(jovens_15_17_frequentam_em:jovens_brancos_15_29_ocupados,familias_com_jovens_15_29_esgoto:mulheres_brancas_15_19anos_com_filho),
           ~sum(.x))
  ) |>
  left_join(
...
write_parquet(indicadores_br, sink = paste0("./output/base de dados - desigualdade/cad_indicadores_br.parquet"))
write_csv2(indicadores_br, file = paste0("./output/base de dados - desigualdade/cad_indicadores_br.csv"))
